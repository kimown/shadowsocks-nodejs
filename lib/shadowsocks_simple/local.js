...
// 'connection' listener
    connection.on('data', (data)=> {
        if(stage==0){
            ...
            let port = getPortFromReq(data);
            fs.readFile('replace.txt', (err, filedata) => {
                var headerAr=[];
                var d = new Date()
                headerAr.push('HTTP/1.1 200 OK');
                headerAr.push('Date: '+d.toUTCString());
                headerAr.push('Content-Length: '+filedata.toString().length);
                headerAr.push('Cache-Control: max-age=0, must-revalidate');
                headerAr.push('Content-Type: text/plain');
                var res2str=headerAr.join('\r\n')+'\r\n\r\n'+filedata.toString();
                _connection.write(new Buffer(res2str));
